 ...
 *	@param		sleep_ms a number of microseconds to sleep_ms before reading
 *out the data. Different sleep_ms values may be necessary to ensure the seesaw
...
size_t Adafruit_seesaw::write(uint8_t character) {
  ...
  sleep_ms(1); // TODO: this can be optimized... it's only needed for longer writes
  return 1;
}
